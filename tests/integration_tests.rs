...
fn test_basic_stow_operation_without_dotfiles() {
    ...
    let dot_bashrc_action_exists = actions.iter().any(|report| 
    ...
    let dot_config_action_exists = actions.iter().any(|report| 
    ...
    let readme_action_exists = actions.iter().any(|report| 
    ...
    let license_action_exists = actions.iter().any(|report| 
...
fn test_basic_stow_operation_with_dotfiles() {
    ...
    let bashrc_action_exists = actions.iter().any(|report| 
    ...
    let config_action_exists = actions.iter().any(|report| 
    ...
    let nvim_init_action_exists = actions.iter().any(|report| 
...
fn test_ignore_patterns_functionality() {
    ...
    let has_readme = actions.iter().any(|r| r.original_action.target_path.ends_with("README.md"));
    assert!(!has_readme, "README.md should be ignored");

    let has_license = actions.iter().any(|r| r.original_action.target_path.ends_with("LICENSE"));
    ...
    let has_backup = actions.iter().any(|r| r.original_action.target_path.ends_with("backup~"));
    assert!(!has_backup, "backup~ files should be ignored by default pattern '.*~'");

    let has_git = actions.iter().any(|r| r.original_action.target_path.to_string_lossy().contains(".git"));
...
fn test_custom_ignore_patterns() {
        ...
        if let Ok(actions) = &actions_result {
            for report in actions {
                 if let Some(item) = &report.original_action.source_item {
                    ...
                    eprintln!("  Action (no source_item): Target: {:?}, LinkTarget: {:?}", report.original_action.target_path, report.original_action.link_target_path);
                }
            }
    ...
    let has_test_script = actions.iter().any(|r| r.original_action.target_path.ends_with("test_script"));
    assert!(!has_test_script, "test_script (bin/test_script) should be ignored by custom pattern 'bin/test_script'");

    let has_bashrc = actions.iter().any(|r| r.original_action.target_path.ends_with(".bashrc"));
    ...
    let has_nvim_init = actions.iter().any(|r| r.original_action.target_path.ends_with(".config/nvim/init.vim"));
...
fn test_multiple_packages_stow() {
    ...
    let p1_bashrc_exists = actions.iter().any(|report| 
    ...
    let p2_bashrc_exists = actions.iter().any(|report| 
...
fn test_dotfiles_processing_edge_cases() {
    ...
    let report_pkg1_dot_file = actions.iter().find(|r| {
    ...
    let expected_target_pkg1_dot_file = target_dir.join(".file");
    assert!(
    ...
    assert!(
        fs::symlink_metadata(&expected_target_pkg1_dot_file).unwrap().file_type().is_symlink(),
    ...
    let report_pkg2_dot_foo_bar = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
    ...
    let expected_target_pkg2_dot_foo_bar = target_dir.join(".foo-bar");
    assert!(
    ...
    assert!(
        fs::symlink_metadata(&expected_target_pkg2_dot_foo_bar).unwrap().file_type().is_symlink(),
    ...
    let report_pkg3_dot_dir_only = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
    ...
    let expected_target_pkg3_dot_dir_only = target_dir.join(".dirOnly");
    assert!(
    ...
    // Note: For CreateDirectory actions, link_target_path is None. original_action.action_type should be CreateDirectory.
    ...
    let report_pkg3_nested_file = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
    ...
    let expected_target_pkg3_nested_file = target_dir.join(".dirOnly/some_file.txt");
    assert!(
    ...
    assert!(
        fs::symlink_metadata(&expected_target_pkg3_nested_file).unwrap().file_type().is_symlink(),
        "Target .dirOnly/some_file.txt for package3 is not a symlink. Report details: {:?}",
        report_pkg3_nested_file
    );
    // Optionally, verify link target for .dirOnly/some_file.txt
    assert_eq!(report_pkg3_nested_file.original_action.action_type, ActionType::CreateSymlink, "ActionType for .dirOnly/some_file.txt should be CreateSymlink");
    ...
    let report_pkg5_nodotprefix_dir = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
            item.package_relative_path == Path::new("nodotprefix") &&
            item.target_name_after_dotfiles_processing == Path::new("nodotprefix") &&
...
fn test_relative_path_calculation_basic() {
    ...
    for report in actions.iter().filter(|r| r.original_action.link_target_path.is_some()) {
        let link_target = report.original_action.link_target_path.as_ref().unwrap();
...
fn test_plan_actions_basic_creation_and_conflict() {
    ...
    let action_file_to_link = actions_empty.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("file_to_link.txt")));
    assert!(action_file_to_link.is_some(), "Action for file_to_link.txt not found");
    assert_eq!(action_file_to_link.unwrap().original_action.action_type, ActionType::CreateSymlink, "Expected CreateSymlink for file_to_link.txt");
    assert!(action_file_to_link.unwrap().original_action.link_target_path.is_some(), "Link target path should exist for CreateSymlink");

    let action_dir_to_create = actions_empty.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("dir_to_create")));
    assert!(action_dir_to_create.is_some(), "Action for dir_to_create not found");
    assert_eq!(action_dir_to_create.unwrap().original_action.action_type, ActionType::CreateDirectory, "Expected CreateDirectory for dir_to_create");
    assert!(action_dir_to_create.unwrap().original_action.link_target_path.is_none(), "Link target path should be None for CreateDirectory");

    let action_nested_file = actions_empty.iter().find(|r| r.original_action.target_path.ends_with(Path::new("dir_to_create/nested_file.txt")));
    assert!(action_nested_file.is_some(), "Action for dir_to_create/nested_file.txt not found");
    assert_eq!(action_nested_file.unwrap().original_action.action_type, ActionType::CreateSymlink, "Expected CreateSymlink for nested_file.txt");
    ...
    let action_conflicting_file = actions_file_conflict.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("file_for_conflict.txt")));
    assert!(action_conflicting_file.is_some(), "Action for file_for_conflict.txt not found in conflict scenario");
    assert_eq!(action_conflicting_file.unwrap().original_action.action_type, ActionType::Conflict, "Expected Conflict for file_for_conflict.txt");
    assert!(action_conflicting_file.unwrap().original_action.conflict_details.is_some(), "Conflict details should be present");
    assert!(action_conflicting_file.unwrap().original_action.link_target_path.is_none(), "Link target should be None for Conflict");
    ...
    let action_conflicting_dir = actions_dir_conflict.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("dir_for_conflict")));
    ...
    assert!(action_conflicting_dir.unwrap().original_action.conflict_details.is_some(), "Conflict details should be present for dir conflict");
    assert!(action_conflicting_dir.unwrap().original_action.link_target_path.is_none(), "Link target should be None for dir Conflict");

    let action_nested_in_conflicting_dir = actions_dir_conflict.iter().find(|r| r.original_action.target_path.ends_with(Path::new("dir_for_conflict/another_nested.txt")));
    assert!(action_nested_in_conflicting_dir.is_some(), "Action for dir_for_conflict/another_nested.txt not found");
    assert_eq!(action_nested_in_conflicting_dir.unwrap().original_action.action_type, ActionType::Conflict, "Expected Conflict for item in conflicting dir");
...
fn test_execute_actions_basic_creation() {
    ...
    let pkg1_name = "pkg_exec_dir";
    let package1_dir = stow_dir.join(pkg1_name);
    ...
    // Plan actions
    let planned_actions1_result = rustow::stow::stow_packages(&config1); // This now plans AND executes
    ...
    let reports1 = planned_actions1_result.unwrap();

    assert_eq!(reports1.len(), 1, "Expected 1 report for single directory creation");
    let report_dir = reports1.iter().find(|r| r.original_action.target_path.ends_with("my_dir")).expect("Report for my_dir not found");
    ...
    let pkg1_sim_name = "pkg_exec_dir_sim";
    let package1_sim_dir = stow_dir.join(pkg1_sim_name);
    ...
    let reports1_sim_result = rustow::stow::stow_packages(&config1);
    assert!(reports1_sim_result.is_ok(), "stow_packages (simulate) for dir creation failed: {:?}", reports1_sim_result.err());
    let reports1_sim = reports1_sim_result.unwrap();

    assert_eq!(reports1_sim.len(), 1);
    let report_dir_sim = reports1_sim.iter().find(|r| r.original_action.target_path.ends_with("my_dir_sim")).expect("Report for my_dir_sim not found");
    assert_eq!(report_dir_sim.status, rustow::stow::TargetActionReportStatus::Skipped);
    assert!(report_dir_sim.message.as_ref().unwrap().contains("SIMULATE"));
    ...
    let pkg2_name = "pkg_exec_file_link";
    let package2_dir = stow_dir.join(pkg2_name);
    ...
    let reports2_result = rustow::stow::stow_packages(&config2);
    assert!(reports2_result.is_ok(), "stow_packages for file link failed: {:?}", reports2_result.err());
    let reports2 = reports2_result.unwrap();
    
    assert_eq!(reports2.len(), 1, "Expected 1 report for file link creation");
    let report_file_link = reports2.iter().find(|r| r.original_action.target_path.ends_with("my_file.txt")).expect("Report for my_file.txt not found");
    assert_eq!(report_file_link.status, rustow::stow::TargetActionReportStatus::Success, "File link creation status not Success");
    let target_file_path = target_dir.join("my_file.txt");
    assert!(target_file_path.exists(), "Target file link my_file.txt was not created");
    assert!(fs::symlink_metadata(&target_file_path).unwrap().file_type().is_symlink(), "Target my_file.txt is not a symlink");
    let link_target = fs::read_link(&target_file_path).unwrap();
    // Assuming relative link from target_dir to stow_dir/pkg_exec_file_link/my_file.txt
    let expected_link_target_relative = PathBuf::from("..").join(stow_dir.file_name().unwrap()).join(pkg2_name).join("my_file.txt");
    assert_eq!(link_target, expected_link_target_relative, "Symlink target is incorrect");

    // --- Scenario 2.1: Create Symlink (File) (Simulate) ---
    let pkg2_sim_name = "pkg_exec_file_link_sim";
    let package2_sim_dir = stow_dir.join(pkg2_sim_name);
    ...
    let reports2_sim_result = rustow::stow::stow_packages(&config2);
    assert!(reports2_sim_result.is_ok(), "stow_packages (simulate) for file link failed: {:?}", reports2_sim_result.err());
    let reports2_sim = reports2_sim_result.unwrap();

    assert_eq!(reports2_sim.len(), 1);
    let report_file_link_sim = reports2_sim.iter().find(|r| r.original_action.target_path.ends_with("my_file_sim.txt")).expect("Report for my_file_sim.txt not found");
    assert_eq!(report_file_link_sim.status, rustow::stow::TargetActionReportStatus::Skipped);
    assert!(!target_dir.join("my_file_sim.txt").exists(), "Target file my_file_sim.txt should not exist in simulate mode");
    config2.simulate = false;

    // --- Scenario 3: Create Symlink (Nested File, Parent Dir Auto-Creation) ---
    let pkg3_name = "pkg_exec_nested_link";
    let package3_dir = stow_dir.join(pkg3_name);
    let nested_parent_dir_in_pkg = package3_dir.join("parent_dir");
    fs::create_dir_all(&nested_parent_dir_in_pkg).unwrap();
    fs::write(nested_parent_dir_in_pkg.join("nested_file.txt"), "Nested Hello!").unwrap();
    ...
    let reports3_result = rustow::stow::stow_packages(&config3);
    assert!(reports3_result.is_ok(), "stow_packages for nested link failed: {:?}", reports3_result.err());
    let reports3 = reports3_result.unwrap();

    // Expected: 1 action for parent_dir (CreateDirectory), 1 for nested_file.txt (CreateSymlink)
    assert_eq!(reports3.len(), 2, "Expected 2 reports for nested link creation (dir + file)");

    let report_parent_dir = reports3.iter().find(|r| r.original_action.target_path.ends_with("parent_dir")).expect("Report for parent_dir not found");
    assert_eq!(report_parent_dir.original_action.action_type, ActionType::CreateDirectory);
    assert_eq!(report_parent_dir.status, rustow::stow::TargetActionReportStatus::Success, "Parent directory creation status not Success");
    let target_parent_dir_path = target_dir.join("parent_dir");
    assert!(target_parent_dir_path.exists(), "Target parent_dir was not created");
    assert!(target_parent_dir_path.is_dir(), "Target parent_dir is not a directory");

    let report_nested_link = reports3.iter().find(|r| r.original_action.target_path.ends_with("nested_file.txt")).expect("Report for nested_file.txt not found");
    assert_eq!(report_nested_link.original_action.action_type, ActionType::CreateSymlink);
    assert_eq!(report_nested_link.status, rustow::stow::TargetActionReportStatus::Success, "Nested link creation status not Success");
    let target_nested_file_path = target_parent_dir_path.join("nested_file.txt");
    assert!(target_nested_file_path.exists(), "Target nested_file.txt was not created");
    assert!(fs::symlink_metadata(&target_nested_file_path).unwrap().file_type().is_symlink(), "Target nested_file.txt is not a symlink");
    let nested_link_target = fs::read_link(&target_nested_file_path).unwrap();
    // Assuming relative link from target_dir/parent_dir to stow_dir/pkg_exec_nested_link/parent_dir/nested_file.txt
    let expected_nested_link_target = PathBuf::from("..").join("..").join(stow_dir.file_name().unwrap()).join(pkg3_name).join("parent_dir").join("nested_file.txt");
    assert_eq!(nested_link_target, expected_nested_link_target, "Nested symlink target is incorrect");

    // --- Scenario 3.1: Nested Link (Simulate) ---
    let pkg3_sim_name = "pkg_exec_nested_link_sim";
    let package3_sim_dir = stow_dir.join(pkg3_sim_name);
    let nested_parent_sim_dir_in_pkg = package3_sim_dir.join("parent_dir_sim");
    fs::create_dir_all(&nested_parent_sim_dir_in_pkg).unwrap();
    fs::write(nested_parent_sim_dir_in_pkg.join("nested_file_sim.txt"), "Nested Sim Hello!").unwrap();

    let mut config3_sim = create_test_config(
        stow_dir.clone(),
        target_dir.clone(),
    ...
    let reports3_sim_result = rustow::stow::stow_packages(&config3_sim);
    assert!(reports3_sim_result.is_ok(), "stow_packages (simulate) for nested link failed: {:?}", reports3_sim_result.err());
    let reports3_sim = reports3_sim_result.unwrap();

    assert_eq!(reports3_sim.len(), 2);
    let report_parent_dir_sim = reports3_sim.iter().find(|r| r.original_action.target_path.ends_with("parent_dir_sim")).expect("Report for parent_dir_sim not found (simulate)");
    assert_eq!(report_parent_dir_sim.status, rustow::stow::TargetActionReportStatus::Skipped);
    let report_nested_link_sim = reports3_sim.iter().find(|r| r.original_action.target_path.ends_with("nested_file_sim.txt")).expect("Report for nested_file_sim.txt not found (simulate)");
    assert_eq!(report_nested_link_sim.status, rustow::stow::TargetActionReportStatus::Skipped);
    assert!(!target_dir.join("parent_dir_sim").exists(), "Target parent_dir_sim should not exist in simulate mode");
    assert!(!target_dir.join("parent_dir_sim/nested_file_sim.txt").exists(), "Target nested_file_sim.txt should not exist in simulate mode");
