...
fn create_test_package(stow_dir: &Path, package_name: &str) -> PathBuf {
    ...
    let mut script_file: File = File::create(bin_dir.join("test_script"))
...
fn test_ignore_patterns_functionality() {
    ...
    let has_readme: bool = actions.iter().any(|r| r.original_action.target_path.ends_with("README.md"));
    assert!(!has_readme, "README.md should be ignored");

    let has_license: bool = actions.iter().any(|r| r.original_action.target_path.ends_with("LICENSE"));
    ...
    let has_backup: bool = actions.iter().any(|r| r.original_action.target_path.ends_with("backup~"));
    assert!(!has_backup, "backup~ files should be ignored by default pattern '.*~'");

    let has_git: bool = actions.iter().any(|r| r.original_action.target_path.to_string_lossy().contains(".git"));
...
fn test_custom_ignore_patterns() {
    ...
    let has_test_script: bool = actions.iter().any(|r| r.original_action.target_path.ends_with("test_script"));
    assert!(!has_test_script, "test_script (bin/test_script) should be ignored by custom pattern 'bin/test_script'");

    let has_bashrc: bool = actions.iter().any(|r| r.original_action.target_path.ends_with(".bashrc"));
    ...
    let has_nvim_init: bool = actions.iter().any(|r| r.original_action.target_path.ends_with(".config/nvim/init.vim"));
...
fn test_dotfiles_processing_edge_cases() {
    ...
    let report_pkg1_dot_file: &rustow::stow::TargetActionReport = actions.iter().find(|r| {
    ...
    let report_pkg2_dot_foo_bar: &rustow::stow::TargetActionReport = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
    ...
    let report_pkg3_dot_dir_only: &rustow::stow::TargetActionReport = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
    ...
    let report_pkg4_nodotprefix_file: &rustow::stow::TargetActionReport = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
    ...
    let report_pkg5_nodotprefix_dir: &rustow::stow::TargetActionReport = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
            item.package_relative_path == Path::new("nodotprefix") &&
            item.target_name_after_dotfiles_processing == Path::new("nodotprefix") &&
    ...
    let report_pkg5_nested_file: &rustow::stow::TargetActionReport = actions.iter().find(|r| {
        r.original_action.source_item.as_ref().map_or(false, |item| {
...
fn test_config_integration_verbosity_and_simulate() {
    ...
    let args: Args = Args {
        ...
        override_conflicts: vec![], defer_conflicts: vec![], ignore_patterns: vec![],
    };
...
fn test_plan_actions_basic_creation_and_conflict() {
    ...
    let actions_empty_result: Result<Vec<rustow::stow::TargetActionReport>, rustow::error::RustowError> = stow_packages(&config_empty_target);
    ...
    let action_file_to_link: Option<&rustow::stow::TargetActionReport> = actions_empty.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("file_to_link.txt")));
    ...
    let action_dir_to_create: Option<&rustow::stow::TargetActionReport> = actions_empty.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("dir_to_create")));
    ...
    let action_nested_file: Option<&rustow::stow::TargetActionReport> = actions_empty.iter().find(|r| r.original_action.target_path.ends_with(Path::new("dir_to_create/nested_file.txt")));
    ...
    let actions_file_conflict_result: Result<Vec<rustow::stow::TargetActionReport>, rustow::error::RustowError> = stow_packages(&config_file_conflict);
    assert!(actions_file_conflict_result.is_ok(), "stow_packages failed for file conflict: {:?}", actions_file_conflict_result.err());
    let actions_file_conflict: Vec<rustow::stow::TargetActionReport> = actions_file_conflict_result.unwrap();

    let action_conflicting_file: Option<&rustow::stow::TargetActionReport> = actions_file_conflict.iter().find(|r| r.original_action.target_path.file_name().map_or(false, |name| name == OsStr::new("file_for_conflict.txt")));
    ...
    let action_nested_in_conflicting_dir: Option<&rustow::stow::TargetActionReport> = actions_dir_conflict.iter().find(|r| r.original_action.target_path.ends_with(Path::new("dir_for_conflict/another_nested.txt")));
...
fn test_execute_actions_basic_creation() {
    ...
    let report_dir_sim: &rustow::stow::TargetActionReport = reports1_sim.iter().find(|r| r.original_action.target_path.ends_with("my_dir_sim")).expect("Report for my_dir_sim not found");
    ...
    let expected_link_target_relative: PathBuf = PathBuf::from("..").join(stow_dir.file_name().unwrap()).join(pkg2_name).join("my_file.txt");
    ...
    let reports2_sim_result: Result<Vec<rustow::stow::TargetActionReport>, rustow::error::RustowError> = rustow::stow::stow_packages(&config2);
    ...
    let report_file_link_sim: &rustow::stow::TargetActionReport = reports2_sim.iter().find(|r| r.original_action.target_path.ends_with("my_file_sim.txt")).expect("Report for my_file_sim.txt not found");
    ...
    let reports3_result: Result<Vec<rustow::stow::TargetActionReport>, rustow::error::RustowError> = rustow::stow::stow_packages(&config3);
    ...
    let report_parent_dir: &rustow::stow::TargetActionReport = reports3.iter().find(|r| r.original_action.target_path.ends_with("parent_dir")).expect("Report for parent_dir not found");
    ...
    let report_nested_link: &rustow::stow::TargetActionReport = reports3.iter().find(|r| r.original_action.target_path.ends_with("nested_file.txt")).expect("Report for nested_file.txt not found");
    ...
    let expected_nested_link_target: PathBuf = PathBuf::from("..").join("..").join(stow_dir.file_name().unwrap()).join(pkg3_name).join("parent_dir").join("nested_file.txt");
    ...
    let reports3_sim_result: Result<Vec<rustow::stow::TargetActionReport>, rustow::error::RustowError> = rustow::stow::stow_packages(&config3_sim);
    ...
    let report_parent_dir_sim: &rustow::stow::TargetActionReport = reports3_sim.iter().find(|r| r.original_action.target_path.ends_with("parent_dir_sim")).expect("Report for parent_dir_sim not found (simulate)");
    assert_eq!(report_parent_dir_sim.status, rustow::stow::TargetActionReportStatus::Skipped);
    let report_nested_link_sim: &rustow::stow::TargetActionReport = reports3_sim.iter().find(|r| r.original_action.target_path.ends_with("nested_file_sim.txt")).expect("Report for nested_file_sim.txt not found (simulate)");
